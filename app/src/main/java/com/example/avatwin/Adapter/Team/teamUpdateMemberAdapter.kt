...
class teamUpdateMemberAdapter():RecyclerView.Adapter<teamUpdateMemberAdapter.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.setItem(item)
        val TeamUpdateFragment = TeamUpdateFragment.getInstance()

        holder.itemView.member_delete_button.setOnClickListener {
            items.remove(item)
            TeamUpdateFragment!!.deleteMember(position)
